...
class BinaryWorkbenchOffsetRegionsDialog(OffsetRegionsRowsMixin, QDialog):
    ...
    def __init__(
        ...
        self.setFixedSize(
            OFFSET_REGIONS_SIZE.DIALOG_WIDTH,
            OFFSET_REGIONS_SIZE.DIALOG_HEIGHT,
        )
        ...
        root.setSpacing(OFFSET_REGIONS_SPACING.ROOT)
        self._build_filter(root)
        self.shell = QFrame(self)
        self.shell.setObjectName("workspace-table-shell")
        layout = QVBoxLayout(self.shell)
        layout.setContentsMargins(*BINARY_WORKBENCH_DIALOG_LAYOUT.PANEL_MARGINS)
    ...
    def _build_entry(self, parent: QVBoxLayout) -> None:
        ...
        row.setContentsMargins(
            BINARY_WORKBENCH_DIALOG_LAYOUT.ZERO,
            BINARY_WORKBENCH_DIALOG_LAYOUT.ZERO,
            _delete_gutter_width(),
            BINARY_WORKBENCH_DIALOG_LAYOUT.ZERO,
        )
        row.setSpacing(BINARY_WORKBENCH_DIALOG_LAYOUT.ROW_SPACING)
        self.name = lba_input(BINARY_WORKBENCH_TEXT.OFFSET_NAME, entry, width=BINARY_WORKBENCH_LAYOUT.SHARED_INPUT_WIDTH)
        self.offset = lba_input(BINARY_WORKBENCH_TEXT.OFFSET_VALUE, entry, width=BINARY_WORKBENCH_LAYOUT.SHARED_INPUT_WIDTH)
        set_hex_value_validator(self.offset)
        configure_binary_workbench_input(self.name, BINARY_WORKBENCH_LAYOUT.SHARED_INPUT_WIDTH)
        configure_binary_workbench_input(self.offset, BINARY_WORKBENCH_LAYOUT.SHARED_INPUT_WIDTH)
        configure_binary_workbench_line_edit(self.name, BINARY_WORKBENCH_LAYOUT.SHARED_INPUT_WIDTH)
        configure_binary_workbench_line_edit(self.offset, BINARY_WORKBENCH_LAYOUT.SHARED_INPUT_WIDTH)
    ...
    def _build_rows(self, parent: QVBoxLayout, regions: list[BinaryWorkbenchOffsetRegionDTO]) -> None:
        ...
        scroll_layout.setContentsMargins(
            BINARY_WORKBENCH_DIALOG_LAYOUT.ZERO,
            BINARY_WORKBENCH_DIALOG_LAYOUT.SCROLL_VERTICAL_MARGIN,
        ...
        self.body_layout.setContentsMargins(*BINARY_WORKBENCH_DIALOG_LAYOUT.EMPTY_MARGINS)
        self.body_layout.setSpacing(BINARY_WORKBENCH_DIALOG_LAYOUT.ROW_SPACING)
        ...
        self.remove_layout.setContentsMargins(*BINARY_WORKBENCH_DIALOG_LAYOUT.EMPTY_MARGINS)
        self.remove_layout.setSpacing(BINARY_WORKBENCH_DIALOG_LAYOUT.ROW_SPACING)
