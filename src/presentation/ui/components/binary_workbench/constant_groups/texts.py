class BINARY_WORKBENCH_TEXT:
    TITLE: str = "Binary Workbench"
    STATUS_IDLE: str = "Ready."
    STATUS_PLACEHOLDER_TEMPLATE: str = '"{name}" is not implemented yet.'
    STATUS_OPENED_TEMPLATE: str = 'Opened "{name}".'
    STATUS_ALREADY_OPEN_TEMPLATE: str = 'File "{name}" is already open.'
    STATUS_INTERNAL_ALREADY_OPEN_TEMPLATE: str = 'Internal file "{name}" is already open.'
    STATUS_CLOSED_TEMPLATE: str = 'Closed "{name}".'
    STATUS_INTERNAL_CLOSED_TEMPLATE: str = 'Closed "{name}". Internal Files.'
    STATUS_INTERNAL_SOURCE_REQUIRED: str = "Select a file-backed tab before opening an internal file."
    STATUS_INTERNAL_FILES_REQUIRED: str = "Configure the LBA File System before opening an internal file."
    STATUS_INTERNAL_REQUIREMENTS: str = (
        "Internal Files requires an open binary file with an LBA File System configured."
    )
    STATUS_BINARY_REQUIRED: str = "Open a binary tab before using this action."
    STATUS_FILE_REQUIRED: str = "Open a file before using this action."
    STATUS_VERSION_REQUIRED: str = "Use or load a version before saving a new binary file."
    STATUS_WORKSPACE_SOURCE_REQUIRED: str = "Save this scratch code before using Versions or Environment data."
    STATUS_SYMBOLS_VERSION_REQUIRED: str = "Save the current version with Alt+S before opening Local Symbols."
    STATUS_ORIGINAL_SAVE_FORBIDDEN: str = "Save File must create a new file. Choose a path different from the original."
    STATUS_NO_VERSIONS: str = "Create or load a version before using this action."
    STATUS_VERSION_CREATED_TEMPLATE: str = 'Created version "{name}".'
    STATUS_VERSION_UPDATED_TEMPLATE: str = 'Updated version "{name}".'
    STATUS_VERSION_LOADED_TEMPLATE: str = 'Loaded version "{name}".'
    STATUS_FILE_SAVED_TEMPLATE: str = 'Saved new binary file "{name}".'
    STATUS_LBA_FILESYSTEM_SAVED_TEMPLATE: str = 'Saved LBA File System "{name}".'
    STATUS_LBA_FILESYSTEM_LOADED_TEMPLATE: str = 'Loaded LBA File System "{name}".'
    STATUS_ASSEMBLY_SAVED_TEMPLATE: str = 'Saved assembly/code file "{name}".'
    STATUS_NOT_FOUND: str = "No match found."
    STATUS_TARGET_PENDING: str = "This target type is available but not implemented yet."
    STATUS_MULTIPLE_TARGETS: str = "Select one result to navigate."
    STATUS_NO_FOCUSED_EDITOR: str = "Focus Bytes or Instruction before saving."
    STATUS_COMMAND_INVALID_INSTRUCTIONS: str = "Command instructions must contain valid complete assembly instructions."
    STATUS_COMMAND_EMPTY_LINES_REQUIRED: str = "Not enough empty lines to apply this command with current Rules."
    STATUS_OFFSET_OUT_OF_RANGE: str = "Jump/branch target is outside the current file."
    STATUS_TARGET_MISALIGNED: str = "Jump/branch target is not aligned to 4 bytes."
    STATUS_BRANCH_OUT_OF_RANGE: str = "Branch target is outside the supported 16-bit relative range."
    STATUS_ASSEMBLY_REFRESH_REQUIRED: str = (
        "Viewport moved beyond the immediate 4 KB repair. Its rebuild was reprioritized; press F1 again if this region still appears stale."
    )
    STATUS_ASSEMBLY_REBUILDING: str = (
        "The visible 4 KB region is ready; the remaining Assembly projection is rebuilding in the background."
    )
    STATUS_MULTILINE_PASTE_LINE_MISMATCH: str = "Paste line count must match the selected line count."
    FILE: str = "File"
    ENVIRONMENT: str = "Environment"
    PREFERENCES: str = "Preferences"
    SEARCH: str = "Search"
    HELP: str = "Guide"
    OPEN_FILE: str = "Open File"
    NEW_SCRATCH_CODE: str = "New Scratch Code"
    OPEN_INTERNAL_FILE: str = "Internal Files"
    VERSION: str = "Version"
    CREATE_VERSION: str = "Create Version"
    ATT_CURRENT_VERSION: str = "Att Version"
    LOAD_VERSION: str = "Load Versions File"
    SAVE_BINARY_FILE: str = "Save File"
    SAVE_ASSEMBLY_CODE: str = "Save Assembly / Code"
    SYMBOLS: str = "Symbols"
    LOCAL_SYMBOLS: str = "Local Symbols"
    GLOBAL_SYMBOLS: str = "Global Symbols"
    LABELS: str = "Labels"
    LBA_FILESYSTEM: str = "LBA File System"
    OFFSET_REGIONS: str = "Offset Regions"
    ENCODING_TABLES: str = "Encoding Tables"
    COMMANDS: str = "Commands"
    VIEW: str = "View"
    ADVANCED_CONFIGURATION: str = "Advanced Configuration"
    BYTES_FORMATTER: str = "Bytes Formatter"
    REFERENCE_OFFSETS: str = "Reference Offsets"
    RULES: str = "Rules"
    GO_TO: str = "Go to"
    FIND: str = "Find"
    HAZARDS: str = "Hazards"
    FIND_HAZARDS: str = "Find Hazards"
    SELECT_BLOCK: str = "Select Block"
    REPLACE_BYTES: str = "Replace Bytes"
    SELECT_ALL: str = "Select All"
    CONFIRM: str = "Confirm"
    FILE_OFFSET: str = "File Offset"
    INSTRUCTION: str = 'Editor (Assembly Code)'
    INSTRUCTIONS: str = "Instructions"
    RAW_INSTRUCTIONS: str = "Raw Instructions"
    DECODED_TEXT: str = "Decoded Text"
    OFFSETS: str = "Offsets"
    BYTES: str = "Bytes"
    AUTO_READ_MODE: str = "Auto"
    BYTES_READ_MODE: str = "Bytes"
    ASSEMBLY_READ_MODE: str = "Assembly Text"
    SELECTION_EMPTY: str = "Selected: none | Length: 0 bytes"
    OFFSET_SUMMARY_EMPTY: str = "Offset (h): --"
    OFFSET_SUMMARY_TEMPLATE: str = "Offset (h): {offset}"
    SELECTED_BLOCK_SUMMARY_EMPTY: str = "Selected Block (h): --"
    SELECTED_BLOCK_SUMMARY_TEMPLATE: str = "Selected Block (h): {first} - {last}"
    LENGTH_SUMMARY_EMPTY: str = "Length (bytes): 0"
    LENGTH_SUMMARY_TEMPLATE: str = "Length (bytes): {length}"
    CPU_ARCH_SUMMARY_TEMPLATE: str = "CPU Arch: {cpu_arch}"
    INTERNAL_FILE_SUMMARY_TEMPLATE: str = 'Internal File from "{name}"'
    FILE_FILTER_ASSEMBLY: str = "Assembly / code files (*.asm *.s *.txt *.c *.cpp *.*)"
    FILE_FILTER_ANY: str = "All supported files (*.asm *.s *.*)"
    FILE_FILTER_OUTPUT: str = "Binary files (*.bin);;All files (*.*)"
    SYMBOLS_TITLE: str = "Symbols"
    SYMBOLS_SUBTITLE: str = "Manage Symbols used by the pre-parser."
    LABELS_SUBTITLE: str = "Inspect labels detected from the current instructions."
    SYMBOL_TYPE: str = "Select"
    SYMBOL_NAME: str = "Name"
    SYMBOL_VALUE: str = "Value"
    SYMBOL_OFFSETS: str = "Offsets"
    SYMBOL_OFFSETS_EMPTY: str = "No offsets found."
    SYMBOL_OFFSETS_STALE: str = "Offsets are outdated for the active tab. Request them again."
    SYMBOL_OFFSETS_TITLE_TEMPLATE: str = "{name} offsets"
    SYMBOL_ADD: str = "Add"
    SYMBOL_REMOVE: str = "Remove"
    FILTER: str = "Filter"
    LABEL_NAME: str = "Label Name"
    OFFSET: str = "Offset"
    ADD_SYMBOL_FROM_IMMEDIATE: str = "Add Symbol"
    CHANGE_SYMBOL_VALUES: str = "Change Symbol Values"
    OPEN_LABEL_NEW_TAB: str = "Open Label in New Tab"
    COLLAPSE_LABEL: str = "Collapse label instructions"
    EXPAND_LABEL: str = "Expand label instructions"
    COLLAPSE_DIRECTIVES: str = "Collapse debugger directives"
    EXPAND_DIRECTIVES: str = "Expand debugger directives"
    ADD_COMMAND: str = "Add Command"
    COMMAND_INSTRUCTION_HEADER: str = "Instruction"
    COMMAND_NAME: str = "Command Name"
    SHOW: str = "Show"
    LOAD: str = "Load"
    LOAD_TABLE: str = "Load Table"
    SAVE: str = "Save"
    OK: str = "OK"
    FILE_FILTER_COMMANDS: str = "Command files (*.json)"
    LABEL_TAB_NAME_TEMPLATE: str = "{label} - {source}"
    PREVIOUS_TAB: str = "Previous tab"
    NEXT_TAB: str = "Next tab"
    LBA_FILE_NAME: str = "File Name"
    LBA_START: str = "LBA Start"
    OFFSET_NAME: str = "Name"
    OFFSET_VALUE: str = "Offset"
    DETAILS: str = "Details"
    CANCEL: str = "Cancel"
    REFERENCE_OFFSETS_SUBTITLE: str = "Configure up to three extra offset columns."
    REFERENCE_NAME: str = "Name"
    REFERENCE_BASE: str = "Base at File 0x0"
    REFERENCE_VISIBLE: str = "Visible"
    REFERENCE_JUMP_TARGET: str = "Jump"
    GO_TO_SUBTITLE: str = "Navigate by offset, LBA, internal file or symbolic target."
    FIND_SUBTITLE: str = "Search assembly, bytes or decoded ANSI text."
    SELECT_BLOCK_SUBTITLE: str = "Select a block by start/end or start/length."
    REPLACE_BYTES_SUBTITLE: str = "Replace bytes using a start offset and optional safety limits."
    REPLACE_BYTES_CONFIRM_SUBTITLE: str = "The target range contains non-zero bytes. Replace them?"
    REPLACEMENT_BYTES: str = "Replacement Bytes"
    STATUS_REPLACE_BYTES_INVALID: str = "Enter valid offsets, limits, and hexadecimal replacement bytes."
    STATUS_REPLACE_BYTES_OUT_OF_RANGE: str = "Replacement range is outside the current file."
    STATUS_REPLACE_BYTES_SUCCESS_TEMPLATE: str = "Replaced {length} bytes at 0x{offset:08X}."
    VALUE: str = "Value"
    START_OFFSET: str = "Start Offset"
    END_OFFSET: str = "End Offset"
    FIND_LENGTH: str = "Length (KB)"
    LENGTH_BYTES: str = "Length Bytes"
    FILE_OFFSET_TARGET: str = "File Offset"
    LBA_2048_TARGET: str = "LBA (2048 bytes)"
    LBA_2334_TARGET: str = "LBA (2334 bytes)"
    LBA_2352_TARGET: str = "LBA (2352 bytes)"
    LABEL_TARGET: str = "Label"
    EQUATE_TARGET: str = "Equate"
    VARIABLE_TARGET: str = "Variable"
    SYMBOL_TARGET: str = "Symbol"
    INTERNAL_FILE_TARGET: str = "Named File / Internal File"
    FIND_ASSEMBLY: str = "Assembly instruction"
    FIND_HEX_BYTES: str = "Hex Bytes (LE)"
    FIND_HEX_BYTES_BE: str = "Hex Bytes (BE)"
    FIND_DECODED_TEXT: str = "Decoded Text"
    FIND_AUTO_FILL_START_OFFSET: str = "Auto-fill Start Offset"
    SAVE_BEFORE_CLOSE: str = "Save changes before closing this tab?"
